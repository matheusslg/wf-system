---
description: Analyze Jira ticket or GitHub issue, fetch Figma designs, break into sub-tasks, create GitHub Issues
allowed-tools: Read, Bash, Grep, Glob, Task, AskUserQuestion
argument-hint: <Jira ticket, GitHub issue number, or URL>
---

# Tech Lead - Issue Planning & Task Breakdown
...
Extract Tech Lead settings:
- `techLead.jiraProject`: Jira project key prefix (e.g., "SXRX")
- `techLead.jiraCloudId`: Atlassian cloud ID or site URL
- `techLead.githubOwner`: GitHub repository owner
- `techLead.githubRepo`: GitHub repository name
- `techLead.defaultLabels`: Default labels for issues
- `agents`: Available agents for delegation

If `techLead` config is missing, ask user for:
- Jira project key
- GitHub owner/repo
...
mcp__github__update_issue(
  ...
  labels: [...existing_labels, "tracked", "tech-lead"]
)
```
...
## Implementation Plan (Tech Lead Breakdown)
...
---
*Breakdown generated by Tech Lead via `/wf-tech-lead`*
```
...
mcp__github__create_issue(
  ...
  labels: ["feature", "tracked", "tech-lead"]
)
```
...
---
*Generated by Tech Lead via `/wf-tech-lead`*
```
...
# Execute a sub-task
/wf-tech-lead-delegate {child_number}
```
```
...
The Tech Lead command requires the GitHub MCP server for creating issues.
...
```bash
# From Jira ticket
/wf-tech-lead SXRX-1023
/wf-tech-lead https://gnarlysoft.atlassian.net/browse/SXRX-1023

# From GitHub issue
/wf-tech-lead #42
/wf-tech-lead 42
/wf-tech-lead https://github.com/owner/repo/issues/42

# With flags
/wf-tech-lead #42 --dry-run
/wf-tech-lead SXRX-1023 --skip-figma
```

## Related Commands
- `/wf-tech-lead-status` - Check implementation progress
- `/wf-tech-lead-delegate` - Execute a specific sub-task with its agent
