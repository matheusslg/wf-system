...
This is the quantitative counterpart to `visual-verify` (which does subjective AI-judged checks). Use this skill when you need exact pixel-level scores, diff images, and structured reports.
...
If none available, warn and exit.

### Figma MCP

Check if `mcp__figma__get_screenshot` is available. If not, warn and exit.
...
2. Read `design.matchFigma.baseUrl` — if missing, ask for base URL
3. For each keyFrame that has a `route` field:
   ...
   - Determine breakpoints and resolve nodeId per breakpoint:
     - If keyFrame `breakpoints` is an **array of strings** → use those breakpoint names, use the keyFrame's top-level `nodeId` for all
     - If keyFrame `breakpoints` is an **object** → use `breakpoints[breakpointName].nodeId` for each
...
| No browser tool available | Warn and exit |
| Figma MCP not available | Warn and exit |
...
| Diff script error | Show error, skip that comparison, continue batch |
| No keyFrames with routes (batch) | Warn: "No keyFrames with routes found" |
| Missing baseUrl (batch) | Ask for base URL |
